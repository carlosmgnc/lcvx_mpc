...
class Plots():
    ...
    def plot(self, sim, traj, sixdof):
        ...
        for i in range(3):
            plt.plot(t_list, x[1 + i, :sim.nsim], label="")
        plt.legend(["x", "y", "z"])
        plt.xlabel("time")
        plt.ylabel("position")
        ...
        plt.plot(t_list, x[0, :sim.nsim], label="")
        plt.xlabel("time")
        plt.ylabel("mass")
        ...
        # plt.plot(t_list[:], np.linalg.norm(u[:, :], axis=0))
        # plt.xlabel("time")
        # plt.ylabel("thrust")
        ...
        anim_int = 100
        animation = FuncAnimation(fig_anim, update, frames=sim.nsim, interval=anim_int)
